...
export default function UpdateIaModal({ isOpen, onClose, initialData, onSuccess }) {
  ...
  useEffect(() => {
    if (initialData) {
      ...
      const statusId = initialData.partnershipStatusId || STATUS_OPTIONS.find(opt => opt.label === initialData?.iaPartnershipStatusName)?.id || 1;
  ...
  const onSubmit = (data) => {
    ...
    dispatch(asyncUpdateImplementationAgreementById({ id: initialData.id || initialData.implementationAgreementsId, payload }))
      .unwrap()
      .then(() => {
        onSuccess?.();
        onClose();
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              <SingleSelectDropdownBadge
                ...
                setValue={setValue}
                isRequired
                defaultValue={initialData?.partnershipStatusId || STATUS_OPTIONS.find(opt => opt.label === initialData?.iaPartnershipStatusName)?.id}
              />
            </div>
