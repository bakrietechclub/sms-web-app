...
export default function UpdateSpkModal({
  isOpen,
  onClose,
  initialData,
  onSuccess,
  ...
  useEffect(() => {
    if (initialData) {
      ...
      const statusId =
        ...
        STATUS_OPTIONS.find(
          (opt) => opt.label === initialData?.spkPartnershipStatus,
        )?.id ||
        1;
  ...
  // Auto-calculate spkDueDate based on spkSignatureDate + spkTimePeriod (in years)
  const spkSignatureDate = watch('spkSignatureDate');
  const spkTimePeriod = watch('spkTimePeriod');

  useEffect(() => {
    if (spkSignatureDate && spkTimePeriod) {
      const dueDate = calculateDueDate(spkSignatureDate, spkTimePeriod);
      if (dueDate) {
        setValue('spkDueDate', formatDateInput(dueDate), {
          shouldValidate: true,
        });
      }
    }
  ...
  const onSubmit = (data) => {
    ...
    dispatch(
      asyncUpdateSpkById({ id: initialData.spkId || initialData.id, payload }),
    )
      .unwrap()
      .then(() => {
        onSuccess?.();
