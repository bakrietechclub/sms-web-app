...
import { ChevronDown, ChevronUp, Search, FileText, CalendarPlus2, Users, MailOpen, MailPlus, Clipboard, Sheet, FileCheck, Trophy } from "lucide-react";
...
const SidebarMenu = () => {
  ...
  const menus = sidebarMenus[role?.toLowerCase()?.includes('media') ? 'media' : role?.toLowerCase()?.includes('ingo') ? 'lembagaInternasional' : 'universitas'];

  return (
    <aside className="w-64 h-screen bg-white border-r border-black fixed flex flex-col">
      {/* Logo BCF */}
      <div className="p-6 text-2xl font-bold text-blue-700 tracking-wide">
        LogoBCF
      </div>
        ...
        {menus.map((menu, idx) => (
          <div key={idx}>
            <div
              ...
              className={clsx(
                "flex items-center justify-between p-3 cursor-pointer hover:bg-gray-100 rounded-md transition-all duration-200",
                openMenu === menu.title && "text-blue-600 font-semibold"
              )}
            >
            ...
            {menu.submenu && openMenu === menu.title && (
              <div className="pl-10 flex flex-col gap-2 py-2">
                {menu.submenu.map((sub, subIdx) => (
                  <div
                    key={subIdx}
                    className="text-gray-500 hover:text-blue-600 text-sm cursor-pointer transition-all duration-200"
                  >
                    {sub}
                  </div>
                ))}
